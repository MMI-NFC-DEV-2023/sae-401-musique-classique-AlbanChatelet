...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><link rel="stylesheet" href="public/styles/global.css">${renderHead()}</head> ${renderComponent($$result, "HeaderPage", $$HeaderPage, {})} ${renderComponent($$result, "Layout", $$Layout, { "pageTitle": "Liste des oeuvres" }, { "default": async ($$result2) => renderTemplate` <section class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 p-6 bg-black"> ${oeuvres.map((oeuvre) => renderTemplate`<div${addAttribute(oeuvre.id, "key")} class="bg-fond_cards p-4 rounded-lg shadow-lg transition duration-300 hover:scale-105"> ${oeuvre.cover && renderTemplate`<img${addAttribute(`${baseUrl}/api/files/oeuvres/${oeuvre.id}/${oeuvre.cover}`, "src")}${addAttribute(`Photo de ${oeuvre.titre}`, "alt")} class="w-full h-auto object-cover rounded-lg mb-4">`} <div class="px-2"> <h2 class="text-2xl font-bold text-white">${oeuvre.titre}</h2> <h2 class="text-xl font-semibold text-[#EFDE7C]">
Par
